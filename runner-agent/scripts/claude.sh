...
claude --model $MODELNAME --output-format stream-json --verbose -p "Work on the current PR."
...
if git diff --quiet HEAD -- ':!.pr'; then
    ...
    if [ ! -f .pr/commit.md ]; then
        claude --model $MODELNAME --output-format stream-json --verbose -c -p "Write a commit message to .pr/commit.md"
    fi
fi
